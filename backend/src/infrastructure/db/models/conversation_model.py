"""SQLAlchemy ORM models for Conversation and Message."""
...
class MessageModel(Base):
    ...
    conversation_id: Mapped[str] = mapped_column(
        CHAR(36), ForeignKey("conversations.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    created_at: Mapped[str] = mapped_column(String(64))  # ISO string for simplicity
...
class ConversationModel(TimestampMixin, Base):
    ...
    lead_id: Mapped[str] = mapped_column(
        CHAR(36), ForeignKey("leads.id", ondelete="CASCADE"), nullable=False, index=True
    )
    agent_id: Mapped[str] = mapped_column(
        CHAR(36), ForeignKey("sales_agents.id"), nullable=False, index=True
    )
    ...
    messages: Mapped[list[MessageModel]] = relationship(
        "MessageModel", back_populates="conversation", lazy="selectin", cascade="all, delete-orphan"
    )
