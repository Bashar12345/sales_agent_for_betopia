...
MILESTONES: list[tuple[str, str]] = [
    (
        "Foundation",
        "Zohra infra + frozen port contracts landed — all agents can start coding",
    ),
    (
        "P1-Live",
...
ISSUES: list[Issue] = [
    ...
    Issue(
        ...
        body=(
            ...
            "_See_ `backend/instruction_zohra.md` § B1"
        ),
        labels=["P1-Blocker", "Zohra-DataFoundation"],
    ...
    Issue(
        ...
        body=(
            ...
            "_See_ `backend/instruction_zohra.md` § B2"
        ),
        labels=["P1-Blocker", "Zohra-DataFoundation"],
        milestone="Foundation",
    ),
    ...
    Issue(
        ...
        body=(
            ...
            "_See_ `backend/instruction_zohra.md` § B3"
        ),
        labels=["P1-Blocker", "Zohra-DataFoundation"],
        milestone="Foundation",
    ),
    ...
    Issue(
        title="[Zohra] T1 — QdrantVectorClient: 3 collections, HNSW + INT8 quantisation",
        body=(
            "**Owner:** Zohra\n\n"
            "## Acceptance\n"
        ...
        labels=["P2-Core", "Zohra-DataFoundation"],
        milestone="Foundation",
    ),

    Issue(
        title="[Zohra] T2 — RedisCacheClient: suggestion + embedding + rate-limit caches",
        body=(
            "**Owner:** Zohra\n\n"
            "## Methods\n"
            "```python\n"
    ...
    Issue(
        title="[Zohra] T3 — NATSClient: CloudEvents 1.0 publish via JetStream",
        body=(
            "**Owner:** Zohra\n\n"
            "## Acceptance\n"
    ...
    Issue(
        title="[Zohra] T4 — indexing_worker: index_conversation + re_embed_collection (p3-batch)",
        body=(
            "**Owner:** Zohra\n\n"
    ...
    Issue(
        title="[Zohra] T5 — Celery factory: p1-high, p2-normal, p3-batch queues",
        body=(
            "**Owner:** Zohra\n\n"
            "## Acceptance\n"
    ...
    Issue(
        title="[Zohra] T6 — GET /health: check PG + Redis + Qdrant + NATS",
        body=(
            "**Owner:** Zohra\n\n"
            "## Acceptance\n"
        ...
        labels=["P3-Quality", "Zohra-DataFoundation"],
        milestone="Foundation",
    ),

    # ── Niloy — P1 Conversation Engine ──────────────────────────────────────

    Issue(
        title="[Niloy] LLM client: GPT-4.1 → Claude Sonnet 4.6 → vLLM fallback chain",
        body=(
            "**Owner:** Niloy — P1 Conversation Engine\n"
            "**Depends on:** Zohra B3 (port interfaces frozen)\n\n"
            "## Acceptance\n"
            ...
            "_See_ `backend/instruction_niloy.md`"
        ),
        labels=["P1-Blocker", "Niloy-ConversationEngine", "blocked"],
        milestone="P1-Live",
    ),

    Issue(
        title="[Niloy] SuggestReplies use case: 9-step P1 pipeline with Redis + Qdrant",
        body=(
            "**Owner:** Niloy\n"
            "**Depends on:** Zohra QdrantVectorClient + RedisCacheClient + NATSClient\n\n"
            "## Pipeline steps\n"
    ...
    Issue(
        title="[Niloy] API routes: POST /input + conversations CRUD (6 endpoints)",
        body=(
            "**Owner:** Niloy\n"
            "**Depends on:** Bashar `require_auth`\n\n"
            "```\n"
        ...
        labels=["P2-Core", "Niloy-ConversationEngine", "blocked"],
        milestone="P1-Live",
    ),

    Issue(
        title="[Niloy] P1 prompt eval suite: ≥10 test cases, CI-gated",
        body=(
            "**Owner:** Niloy\n\n"
            "## Acceptance\n"
        ...
        labels=["P3-Quality", "Niloy-ConversationEngine"],
        milestone="P1-Live",
    ),
    ...
    Issue(
        ...
        body=(
            ...
            "_See_ `backend/instruction_bashar.md` § BLOCKING"
        ),
        labels=["P1-Blocker", "Bashar-FrontendAuth"],
        milestone="Foundation",
    ),

    Issue(
        title="[Bashar] Auth endpoints: POST /auth/login, /auth/refresh, GET /auth/me",
        body=(
            "**Owner:** Bashar\n\n"
            "```\n"
    ...
    Issue(
        title="[Bashar] Lead CRUD: POST/GET/PATCH/DELETE /api/v1/leads",
        body=(
            "**Owner:** Bashar\n"
            "**Depends on:** Bashar `require_auth` (self-dependency)\n\n"
            "```\n"
        ...
        labels=["P2-Core", "Bashar-FrontendAuth", "blocked"],
        milestone="Foundation",
    ),

    Issue(
        title="[Bashar] Fiverr webhook: create lead + publish NATS leads.created",
        body=(
            "**Owner:** Bashar\n\n"
    ...
    Issue(
        title="[Bashar] CI workflow: ruff + mypy + pytest + prompt evals + frontend build",
        body=(
            "**Owner:** Bashar\n\n"
        ...
        labels=["P3-Quality", "Bashar-FrontendAuth"],
        milestone="Production",
    ),

    Issue(
        title="[Bashar] CD workflow + K8s manifests: build → push → kubectl apply",
        body=(
            "**Owner:** Bashar\n\n"
            "## CD (on merge to main)\n"
            "```yaml\n"
